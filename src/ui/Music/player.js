...
var Tracker = {
	...
	isOusideOfBigCircle: function (e) {
		return (
			Math.abs(e.pageX - this.scene.cx - this.scene.coord.left) > this.scene.radius ||
	...
	draw: function () {
		...
		if (!this.pressButton) {
			this.angle =
				(Player.context.currentTime / Player.source.buffer.duration) * 2 * Math.PI || 0;
		}
		this.drawArc();
	},
...
var Player = {
	...
	loadTrack: function (song) {
		...
		document.getElementById(`loading${track.id}`).innerHTML =
			'<img src="https://kotzevs-v2.s3.eu-central-1.amazonaws.com/photos/loading.gif" style="width: 40px; height: 30px;"/>';
